# OurWork1：基于 CoFE 去噪增强的行人轨迹预测网络

OurWork1 是在 PTINet 基础上扩展的轨迹预测框架，专注于 **FPV（第一人称视角）轨迹预测任务**。相比于原始 PTINet，本项目引入了 **CoFE（Correction Feature Embedding）轨迹去噪修复模块**，在历史轨迹送入 LSTM-VAE 编码之前先进行去噪修复，并确保修复后位置与速度的物理一致性。

该框架主要支持 **FPV（第一人称视角）**（T2FPV 数据集），同时保留 BEV（鸟瞰图）视角兼容性。
...
输入轨迹 → [CoFE 轨迹去噪] → 位置编码器(LSTMVAE)
                              → 速度重计算(一阶差分)
输入速度 → 速度编码器(LSTMVAE)
输入图像 → ConvLSTM → 全局特征(GF)                         → 特征融合 → 自回归解码 → 速度预测
输入光流 → ResNet + LSTM
输入意图 → MLP 投影（可选）
```
...
CoFE 基于 GRU 编码器-解码器架构，支持 **FPV** 和 **BEV** 两种模式：
...
| 输入模态 | 说明 | 编码方式 |
|---------|------|---------|
| **轨迹位置** | 历史世界坐标 (x, y) | LSTMVAE 编码器 |
| **轨迹速度** | 帧间差分速度序列 (x, y) | LSTMVAE 编码器 |
...
| **himg** | Hidden - **Im**a**g**e | 图像编码器（ConvLSTM） | 图像序列的最终隐藏状态（池化 + 线性映射后，512 维） | `(batch, hidden_size)` |
| **cimg** | **C**ell - **Im**a**g**e | 图像编码器（ConvLSTM） | 图像序列的最终细胞状态（池化 + 线性映射后，512 维） | `(batch, hidden_size)` |
...
FPV 模式下图像和光流的编码方式与 BEV 模式相同，统一使用 ConvLSTM：

| 变量名 | 全称 | 来源 | 含义 | 形状 |
|--------|------|------|------|------|
| **himg** | Hidden - **Im**a**g**e | `clstm`（ConvLSTM） | FPV 模式下图像序列的最终隐藏状态（池化 + 线性映射后） | `(batch, hidden_size)` |
| **cimg** | **C**ell - **Im**a**g**e | `clstm`（ConvLSTM） | FPV 模式下图像序列的最终细胞状态（池化 + 线性映射后） | `(batch, hidden_size)` |
...
| 变量名 | 全称 | 含义 | 计算方式 |
|--------|------|------|---------|
| **hds** | Hidden **D**ecoder **S**um | 解码器初始隐藏状态（融合所有模态的隐藏特征） | `hpo + hsp + hpa + hsa + pb + himg + hop + intent_emb` |
| **zds** | **Z** **D**ecoder **S**um | 解码器初始细胞状态（融合所有模态的细胞/隐变量特征） | `zpo + zsp + zpa + zsa + pb + cimg + cop + intent_emb` |
...
| `lcf_feature_dim` | LCF（局部上下文特征）维度（0 表示不使用） | 0 |
...
模型为 LCF 部分预留了灵活接口，支持未来特征内容的调整和优化：

- **配置参数**：`lcf_feature_dim` 控制是否启用 LCF 特征
- **输入参数**：`lcf_feature` 可传入任意维度的局部上下文特征
- **投影模块**：`self.lcf_proj` MLP 将 LCF 特征映射到 hidden_size 维度
- **特征融合**：在解码器初始状态融合时参与计算

### 图像特征提取方式选择

支持两种图像特征提取方式，通过 `image_network` 配置参数选择：

| 选项 | 说明 | 适用场景 |
|------|------|---------|
| `clstm` | ConvLSTM 时空卷积 | BEV 模式，需要端到端训练 |
| `resnet50` / `resnet18` | ResNet + LSTM | FPV 模式，使用预提取特征 |
...
- ✅ 重构 `network_image.py`，统一 `forward` 函数，支持 FPV 和 BEV 模式
- ✅ 移除 BEV 专用数据集（JAAD、PIE、TITAN）和预处理脚本
- ✅ 清理 BEV 专用评估指标（AIOU、FIOU）
- ✅ 为 LCF（局部上下文特征）预留灵活接口
- ✅ 更新配置文件，添加 `lcf_feature_dim` 参数
- ✅ 修复 ResNet 命名冲突问题
- ✅ 优化 train.py 数据加载，使用 `get()` 安全访问

### 2024-05-24

- ✅ 简化训练流程，移除未使用的意图预测变量
- ✅ 添加配置警告（LCF 和 Intent 特征未启用时的运行时警告）
- ✅ 优化代码安全性（`getattr` 替代 `eval`）
- ✅ 修复验证集分布式采样器配置（`val_sampler.set_epoch(epoch)`）
- ✅ 移除未使用的 `crossing_loss` 和 `intent_acc` 变量
- ✅ 统一配置文件路径为相对路径
