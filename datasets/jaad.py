...
class JAAD(torch.utils.data.Dataset):
    ...
    def _load_image_op(self, image_path_op):
        flow = np.load(image_path_op)
        flow = torch.from_numpy(flow).permute(2, 0, 1).float()
        flow = F.interpolate(flow.unsqueeze(0), size=(self.image_resize[0], self.image_resize[1]), mode='bilinear', align_corners=False).squeeze(0)
        flow_4ch = torch.cat([flow, flow], dim=0)
        return flow_4ch
    ...
    def __getitem__(self, index):
        ...
        if self.use_opticalflow==True:
            ...
            image_paths_optical= []
            for ul in op_path:
                image_paths_optical.append(ul.replace('images', 'opticalflow').replace('.jpg', '.npy'))
